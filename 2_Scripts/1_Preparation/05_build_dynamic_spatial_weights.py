...
def build_dynamic_spatial_lags(df_panel, df_spatial, W_matrix, prefix="w6"):
    ...
    cols_to_drop = ['year_first_awarded', 'year_first_operating', 'IV_Z_R1', 'IS_R1_LOSER', 'is_r1_third_party', 'wri_first_oem']
    df_spatial = df_spatial.drop(columns=[col for col in cols_to_drop if col in df_spatial.columns])

    df_spatial = df_spatial.merge(base_df[['nces_id', 'year_first_awarded', 'year_first_operating', 'IV_Z_R1', 'IS_R1_LOSER', 'is_r1_third_party', 'wri_first_oem']], on='nces_id', how='left')
    ...
    is_r1_loser  = df_spatial['IS_R1_LOSER'].fillna(0).values
    is_r1_third_party = df_spatial['is_r1_third_party'].fillna(0).values

    # Split R1 winners by applicant type
    is_r1_winner_self = ((is_r1_winner == 1) & (is_r1_third_party == 0)).astype(float)
    is_r1_winner_third = ((is_r1_winner == 1) & (is_r1_third_party == 1)).astype(float)

    # Application density: districts that applied to R1 (winners OR losers)
    # W @ is_r1_applicant is the Bonhomme-Weidner control for application clustering.
    is_r1_applicant = np.clip(is_r1_winner + is_r1_loser, 0, 1).astype(float)
    w_Z_app_R1 = W_matrix.dot(is_r1_applicant)  # static — computed once outside year loop
    
    # Pre-cache OEM match matrix vector to run quickly
    oems = df_spatial['wri_first_oem'].fillna('Unknown').values
    same_oem_lag = np.zeros(N)
    for oem in np.unique(oems):
        if oem == 'Unknown':
            continue
        # Neighbors who won R1 WITH THIS OEM
        oem_r1_winners = ((is_r1_winner == 1) & (oems == oem)).astype(float)
        # Computes number (or share) of neighbors who won R1 with this specific OEM
        oem_lag = W_matrix.dot(oem_r1_winners)
        # Apply to focal districts who ended up purchasing that same OEM
        idx = (oems == oem)
        same_oem_lag[idx] = oem_lag[idx]
        
    # An indicator purely: "Has at least one R1-winning neighbor with the same OEM"
    # Even if row-standardized, it will be >0 if any valid neighbor has the same OEM
    has_same_oem_r1_neighbor = (same_oem_lag > 0).astype(int)
    ...
    for t in years:
        ...
        if t - 1 >= 2022:
            ...
            Z_self_t_minus_1 = is_r1_winner_self.astype(float)
            Z_third_t_minus_1 = is_r1_winner_third.astype(float)
        else:
            Z_t_minus_1 = np.zeros(N)
            Z_self_t_minus_1 = np.zeros(N)
            Z_third_t_minus_1 = np.zeros(N)
        ...
        w_Z_self = W_matrix.dot(Z_self_t_minus_1)
        w_Z_third = W_matrix.dot(Z_third_t_minus_1)
        ...
        temp = pd.DataFrame({
            ...
            f'{prefix}_Z_self_t_minus_1': w_Z_self,
            f'{prefix}_Z_third_t_minus_1': w_Z_third,
            f'{prefix}_Z_app_R1': w_Z_app_R1,
            f'{prefix}_same_oem_r1': same_oem_lag,
            f'{prefix}_has_same_oem_r1': has_same_oem_r1_neighbor
        })
