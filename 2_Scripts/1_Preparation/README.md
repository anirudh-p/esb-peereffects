...
- `03_build_spatial_design_exposure.py`: builds geometry coverage diagnostics, focal-unit/sample flags, hybrid and EDGE-only KNN graphs, spatial exposure variables, the simple priority-cell BH R1 design layer, and the simulated R1 selection-rule recentered design layer used in Spec C.
...
`03_build_spatial_design_exposure.py` writes spatial/design panel files to `1_Data/Cleaned` and geometry/sample-drop diagnostics to `3_Output/Audit`. It also writes `r1_design_bh_priority_rates.csv`, `r1_design_sim_summary.csv`, and application/district probability audits for the simulated R1 design layer.
